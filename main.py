...
def covid_classifier_model2(img_path, filename):
    ...
    #HEADERS = {'content-type': 'application/json', 'Host': 'dmoons-kim-nb.pjt-dmoons-kim.sds.swa-kubeflow.com'}
    #MODEL2_API_URL = 'https://dmoons-kim-nb-covid-test.pjt-dmoons-kim.sds.swa-kubeflow.com/v1/models/dmoons-kim-nb-covid-test:predict'

    HEADERS = {'content-type': 'application/json', 'Host': 'dmoons-kim-nb-covid-test.pjt-dmoons-kim.sds.swa-kubeflow.com'}
    #HEADERS = {'content-type': 'application/json'}
    MODEL2_API_URL = 'https://123.37.4.53/v1/models/dmoons-kim-nb-covid-test:predict'

    CLASS_NAMES = ['Covid19', 'Normal_Lung', 'Pneumonia_Bacterial_Lung']

    logging.warning("****** Tenserflow Serving Request  *****")
    json_response = requests.post(MODEL2_API_URL, data=data, headers=HEADERS, verify=False) 
